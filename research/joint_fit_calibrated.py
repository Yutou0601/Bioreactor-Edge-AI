...
def figures(ks, pes, rb2, peq2, r2, thj, null2, nullj, R, p2, pj):
    ...
    ax.annotate(f'實測 {rb2:.4f}\np = {p2:.3f}', xy=(rb2, 0), xytext=(-8, 30),
                ha='right', textcoords='offset points', fontsize=10,
                color=RED, fontweight='bold')
    ax.axvline(np.median(null2), color=INK2, ls=':', lw=1.8)
    # 兩條垂直線只差 0.004，標註直接掛在線上會互相疊字；改放左上空白區。
    ax.annotate(f'虛無中位 {np.median(null2):.4f}（假 rb，比實測還大）',
    ...
    # 這個百分比先前是寫死的（97%），資料更新後就跟正文對不上；改為即時計算。
    bias_red = (1 - np.median(nullj) / max(np.median(null2), 1e-9)) * 100
    # 標題只能宣稱「對同模型虛無」——換成雙指數虛無後即不再顯著，見 fig30。
    style(ax, f'(c) 一步聯合擬合：對同模型虛無偏誤降低 {bias_red:.0f}%',
          '回收的 rb (kg/cm²/hr)', '次數')
    ax.legend(frameon=False, fontsize=9)
    ...
    style(ax, '(d) 通道分解（P=1.05，條件於單指數模型成立）',
          '循環時間 τ', '速率 (kg/cm²/hr)')
    ax.legend(frameon=False, fontsize=9)
    ...
    fig.text(0, -0.02,
             ...
             f'(b) 顯示 rb≡0 的合成資料回收出更大的假 rb。改用一步聯合擬合後 (c) 偏誤降低 {bias_red:.0f}%，'
             f'實測 rb={thj[4]:.4f} 對「同模型虛無」顯著大於 0。\n'
             '合成資料以區塊自助保留原始殘差的自相關結構，故本檢定對「估計量是否從噪聲造出 rb」為保守。\n'
             '★ 但 (b)(c) 的虛無皆由同一個單指數模型產生，無法認證模型本身：'
             '改用擬合更好的雙指數（rb≡0）虛無後，實測值即落入分佈之內（p≈0.4，見圖30）。',
