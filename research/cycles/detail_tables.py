...
def data_B():
    ...
    for nm, tau, _, _ in BATCHES:
        ...
        foot.append('%s（宣稱 τ=%d）泵窗 第 %d~%d 分＝%d 分鐘；佔總壓降 %.0f%%；'
                    '泵開 %.4f vs 泵停 %.4f＝%.0f 倍'
...
def data_C():
    ...
    return {
        'title': '表 C　τ=10 自動循環的內部剖面（13 段相干疊加，每 30 分一格）',
...
def data_F():
    ...
    return {
        'title': '表 F　三批次總表（τ 槓桿）',
        'header': ['批次', 'τ分', '期間', '天數', '段數', '降幅中位',
                   '時長中位', '速率中位', '泵窗', '佔壓降', 'CH4峰'],
        'rows': rows,
        'widths': [16, 12, 26, 14, 14, 20, 20, 20, 26, 18, 16],
        'foot': ('「泵窗」＝驟降尖峰至回升尖峰的分鐘區間，其長度應等於 τ。'
