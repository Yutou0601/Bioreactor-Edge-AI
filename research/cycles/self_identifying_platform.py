...
SPAN_DAYS = 12.0
N_OBSERVED = 26          # 實際收到的完整循環數；超過此值的格點只能靠放回抽樣


def learning_curve(D, seed=1):
    """n <= 26 是對實測循環的次抽樣，屬觀測；n > 26 只能靠取後放回把 26 個循環
    膨脹成更大的假樣本，其變異數必然依 1/sqrt(n) 收縮——那是重抽的機械後果，
    不是「多收真實循環會帶來新資訊」的證據。故標記為 extrapolated，
    冪次迴歸與外推一律排除，否則等於把要證明的 -0.5 律先塞進資料裡。"""
...
def main():
    ...
    # 只用觀測點擬合。含 n=40/60 會把放回抽樣的機械 1/sqrt(n) 塞進迴歸，變成循環論證。
    obs = ~L.extrapolated
    sl, ic = np.polyfit(np.log(L.n[obs]), np.log(L.sd[obs]), 1)
    sl_all, _ = np.polyfit(np.log(L.n), np.log(L.sd), 1)
    print(f'\n   精度冪次 = {sl:.3f}   （獨立樣本理論值 -0.50；'
          f'僅用 n<={N_OBSERVED} 的 {int(obs.sum())} 個觀測點）')
    print(f'   對照：若誤把 n>{N_OBSERVED} 的放回抽樣點也納入迴歸，會得到 {sl_all:.3f}'
          f'——該值偏大是重抽的機械後果，不可引用')
    ok = -0.7 < sl < -0.3
...
def figures(D, L, sl, ic, rate, proj, mu):
    ...
    obs = ~L.extrapolated
    ax.loglog(L.n[obs], L.cv[obs] * 100, 'o-', ms=7, lw=2, color=BLUE,
              label=f'實測 CV（n ≤ {N_OBSERVED}）')
    # 放回抽樣點另畫成空心，且不參與擬合——避免被誤讀為觀測。
    ax.loglog(L.n[~obs], L.cv[~obs] * 100, 'o', ms=7, mfc='none', mew=1.6,
              color=MUTED, label='放回抽樣（非觀測，未納入擬合）')
    ...
    fig.text(0, -0.02,
             f'關鍵證據是 (b) 的冪次 {sl:.3f} 幾乎等於獨立樣本的理論值 -0.50——'
             '代表每個循環貢獻「近乎獨立」的資訊，而非重複量測同一個漂移中的系統\n'
             '（後者的精度會很快飽和）。故本反應器的常規運轉本身即為一座持續累積辨識能力的平台：'
             f'不中斷生產、不注入測試訊號，約 {proj[0.10]/rate:.0f} 天即可把估計量的 CV 壓到 10% 以下。\n'
             '★ 這裡量的是**精度**（估計值的離散程度），不是**正確性**：圖30 顯示 rb 本身'
             '在單指數與雙指數模型之間並未被辨識，故本圖應讀作「平台累積資訊的速率」，'
             '不可讀作「rb 已被可靠測定」。\n'
