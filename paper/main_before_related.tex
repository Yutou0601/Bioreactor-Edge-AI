...
physically independent observable. That is why the degeneracy is rarely confronted directly. Where the
quantity of interest is too costly or too slow to instrument
continuously---so that it reaches the operator through offline assay
rather than through a sensor---the field increasingly infers it from
the measurements already available \cite{rutland2025application}. Our
setting is harder in one respect and easier in none: there is no
offline assay to learn from, so the inference cannot be supervised, and
without the option of a second channel the identifiability question
is unavoidable.
...
reference itself is constructed. Line~2 refits each cycle with the rate
...
That spread is wider than the $\pm 11.0\%$ attached to
...
with $\beta$ and reaches $30\%$ at the top of the compatible set. It is
