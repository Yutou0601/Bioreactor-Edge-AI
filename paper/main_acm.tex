% ICEA 2026 — ACM acmart(sigconf) 版，2026-08-23 自 main.tex 轉出
%
% ⚠ main.tex（Springer llncs）保留未動。版式待程序主席確認後擇一。
% ⚠ 本機無 LaTeX toolchain，本檔必須在 Overleaf 編譯驗證。
% ⚠ 上限 10 頁含表圖附錄與參考文獻（CFP 明載，非先前記載的 11 頁）。
%
% 待辦：① dl.acm.org/ccs 產生 CCSXML 區塊貼入
%       ② 編譯後檢視哪些 figure* 其實收得進單欄
%
% 參考文獻用 ACM-Reference-Format + refs.bib（21 筆，與正文同步）。
% Overleaf 會自動跑 bibtex；.bst 由 acmart 提供，不必另外上傳。
%
...
%   · 不得寫成模擬器已充分——殘餘 AUC 0.706（2026-08-24 去重後重算）
%
...
\usepackage{mhchem}
\usepackage{booktabs}
% amsmath／hyperref 由 acmart 載入，勿重複；
% algorithm／algpseudocode 全篇未用（演算法以 verbatim 呈現），已移除。
...
% ── 大會資訊 ──────────────────────────────────────────────
% sigconf 預設每列三位作者，第四位會掉到第二列。acmart 提供
% authorsperrow 直接指定，不必自行改 \author 的排版。
\settopmatter{authorsperrow=4}

\copyrightyear{2026}
\acmYear{2026}
\setcopyright{acmlicensed}
\acmConference[ICEA 2026]{2026 International Conference on
  Intelligent Computing and its Emerging
  Applications}{November 19--22, 2026}{Kaohsiung, Taiwan}
...
\author{Cheng-Yu Li}
\affiliation{%
  \institution{Department of Computer Science and Information
...
\author{Cheng-Yuan Hung}
\affiliation{%
  \institution{Opto-Electronics Technology Section, Energy and
    Agile System Department, Metal Industries Research \& Development Centre}
  \city{Kaohsiung}
  \country{Taiwan}}

\author{Yen-Jie Huang}
\affiliation{%
  \institution{Opto-Electronics Technology Section, Energy and
    Agile System Department, Metal Industries Research \& Development Centre}
  \city{Kaohsiung}
...
and removed rather than assumed away. Across 207 screened
cycles the calibration rules out a constant biological rate and returns
a corrected consumption rate of $0.0125$~\ratu{},
accurate to 11 percent, of which 2 percent is systematic and does
...
% ── CCS Concepts ─────────────────────────────────────────
% ⚠ 下列 \ccsdesc 足以在 PDF 上正確排出概念與權重；
%   投稿前仍須到 https://dl.acm.org/ccs 產生對應的 CCSXML 區塊
%   貼在此處，供 ACM 收錄後設資料之用。
...
\keywords{Process data mining; Practical identifiability;
Anaerobic digestion; Quantized sensing; Biomethanation}
...
\item \textbf{A cycle-level estimation procedure.} A curvature screen
computed from the raw trajectory rejects cycles in which the two contributions
...
\begin{figure*}[t]
\centering
\includegraphics[width=120mm]{equipment.png}
\caption{The instrument as built. The panel carries the touch-panel
controller running the plant program, the pH and oxidation--reduction
potential meters, and the two optical gas analysers reading
...
transducers, oxidation--reduction potential (ORP), and pH. The gas-analyzer
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figD_device_pipeline.pdf}
\caption{Four days of measured pressure; red lines mark
threshold-triggered refills, about twice a day and $270$ times in
...
rule on the rise above the running minimum yields $270$ distinct
descents.

Those cycles are long and shallow. Pressure is logged once a minute, so
a cycle of median duration $10.2$~hr carries roughly $610$ samples. The
median descent amplitude is $0.27$~\pu{}, which is $27$ quantization
steps, while the median residual scale about a fitted trajectory is
$0.0056$~\pu{}, below a single step. The biological term accounts for
roughly half of the descent, so the quantity to be recovered spans on
the order of thirteen steps against a per-sample noise smaller than
one.
...
The estimation geometry we face is fifty-nine years old.
Bandyopadhyay, Humphrey and Taguchi addressed the problem of measuring
a volumetric oxygen transfer coefficient in a fermenter whose culture
keeps consuming oxygen throughout the measurement, and proposed the
dynamic gassing-out method as the
solution \cite{bandyopadhyay1967dynamic}. The procedure is to interrupt
the aeration briefly, trace the dissolved oxygen concentration of the
broth across that interruption, and analyse the resulting trace for
the coefficient $k_{L}a$. One scalar channel therefore has to carry both a transfer
term and a biological one, which is the structure we inherit and in
which we claim no novelty. The authors report the technique as rapid
and internally consistent, and they probe its limit with prolonged
starvation experiments, finding that the oxygen uptake rate itself
changes once the interruption lasts between two and ten minutes. Our
regime differs in two ways: their protocol imposes the interruption
where ours may not perturb a production vessel, and they report a point
estimate where we report a degeneracy-calibrated aggregate.

Linek, Bene\v{s} and Vacek attacked a specific failure of such dynamic
methods \cite{linek1989dynamic}. Their target was the error introduced
when the transient is created by a step change in the oxygen
concentration of the entering gas: interfacial nitrogen transport and
the finite rate at which the concentration change propagates into
individual bubbles can distort the reported coefficient by more than
hundreds of percent. Their solution replaces that step with a change of
roughly twenty percent in the total reactor pressure, which alters the
oxygen concentration in every bubble of the dispersion simultaneously
and so suppresses the influence of non-ideal mixing in the gas phase.
They validate the procedure against two independent references, the
standard dynamic method in which pure oxygen is absorbed into a
previously degassed liquid, and the steady-state sulphite method, and
they describe a test for the one requirement the method imposes, that
the oxygen probe signal be independent of pressure. The method is
reported as suitable for large-scale reactors because the necessary
pressure changes are small and ordinary air can be used. Both
references it is checked against are chemical rather than biological,
which is the point of contact with our problem: the transient is
imposed rather than observed, and the biological term is not among the
...
Hafner, Rennuit, Olsen and Pedersen took as their problem the leaks
that undermine batch biogas assays, and proposed detecting and
correcting them from the change in bottle mass during
incubation \cite{hafner2018quantification}. Three experiments were run
to test the method, on pure chemicals, on wastewater sludge and on
other complex substrates, and leaks were found in all three; their
frequency and magnitude tracked headspace pressure and the number of
times the bottle septa had been punctured. Comparison against an
independent estimate of leakage in two of the experiments showed the
mass-based estimate to be accurate. The reported sensitivity is that
leaks as small as twenty percent of total biogas or methane production
can be detected, and smaller ones when production is high relative to
the precision of the mass measurement, on the strength of which the
authors recommend that the measurement always be made. What that
method supplies is a second and physically independent observable, and
that is why the degeneracy is rarely confronted directly; it is also
precisely what our vessel cannot give us.

Where the quantity of interest is too costly or too slow to instrument
continuously, the field instead infers it from measurements already
available. Rutland, You, Liu and Bowman addressed the FOS/TAC ratio,
which sets the volatile fatty acids of a digester against its
buffering capacity and is used as an indicator of process stability;
its continuous monitoring is limited by the cost and calibration
burden of live sensors and by the delay of offline laboratory
analysis \cite{rutland2025application}. Their solution is a soft sensor
trained on variables that are already logged online, using data from a
one-hundred-and-sixty-day pilot trial of a microbial-electrolysis-cell
digester treating brewery wastewater. The procedure first reduces the
variable set to seven by random-forest feature importance, with pH
ranking highest and hydrogen sulphide second, and then compares
decision trees, XGBoost, support vector regression and artificial
neural networks under five-fold cross-validation in which the temporal
order of the record is preserved rather than shuffled. The neural
network performed best, reaching an explained variance of $0.77$; an
out-of-fold ensemble of that architecture averaged $0.62$ across folds,
or $0.74$ once the single fold containing a loading and temperature
excursion is set aside. The authors also report where the model
degrades, with accuracy falling from $0.79$ to $0.58$ and $F_1$ from
$0.89$ to $0.73$ once the ratio exceeds $0.6$. Ours is an inference of
that kind, but with no offline assay for \rb{} it cannot be supervised,
and without a second channel the identifiability question is
unavoidable.

\subsubsection{Identifiability diagnostics.}
Raue and colleagues addressed the question of how well the parameters
of a partially observed dynamical model are determined by the data at
hand, and answered it by exploiting the profile
likelihood \cite{raue2009structural}, which re-optimises the remaining
parameters while one is held fixed across a range of values. They
report that the approach detects structural non-identifiabilities,
which manifest as functionally related model parameters, and also
practical non-identifiabilities, which arise from the limited amount
and quality of the experimental data. Confidence intervals follow from
the same computation. The authors describe the results as easy to
interpret and directly usable both for planning further experiments and
for reducing the model, and they provide an implementation for MATLAB
and the PottersWheel toolbox.

Simpson and Baker have since set the same territory out as a
self-contained course, aimed at the gap between the applied-statistics
treatment of identifiability and the differential-equation models
applied mathematicians actually
use \cite{simpson2026parameter}. Their approach is likelihood-based
throughout: a process model and a noise model together give a
log-likelihood, numerical optimisation gives the maximum-likelihood
estimate, and univariate profiles---each obtained by holding one
parameter fixed on a grid while the others are optimised out---are
thresholded against a $\chi^{2}$ quantile to give confidence
intervals, which are then propagated into prediction intervals by
sampling parameter sets from inside the confidence set. Three
exercises carry this through an ordinary differential equation, a
partial differential equation and a boundary-value problem, supported
by open-source code in Julia and Python. The first two problems are
identifiable; the third is not, and it is the one that resembles ours.
There the steady-state solution depends on the parameters only through
the combinations $J/\sqrt{kD}$ and $\sqrt{k/D}$, so the univariate
profiles for $J$, $D$ and $k$ come out flat---infinitely many triples
fit the data equally well---whereas reparameterising in terms of those
two combinations restores single-peaked profiles and finite intervals.
The authors also caution that local diagnostics built from the Hessian
at the estimate can mislead precisely when a problem is
non-identifiable, which is why they treat the profile as a global
method. Our degeneracy is of that kind:
Eq.~\eqref{eq:degen} identifies only the sum $Ak+\rb$, and the coupled
alternative discussed in Sect.~\ref{sec:method} identifies only
$P_{eq}-\rb/k$.

The specific ill-conditioning we face was characterized by Varah, whose
subject was the fitting of discrete data or a continuous function by
least squares using exponential functions \cite{varah1985fitting}. He
examined the uniqueness and the sensitivity of the best least-squares
solution and gave both analytic and numerical examples exhibiting
possible non-uniqueness and extreme sensitivity in those solutions.
That result is the mathematical root of the degeneracy in
Eq.~\eqref{eq:le} rather than a defect of our implementation, since any
model containing an exponential inherits it.

Both diagnostics, however, are computed \emph{from} a fit, which is the
gap our screen addresses: we must decide which cycles are worth fitting
before we have a fit to diagnose.
...
Two preparation steps come before any fitting. Their thresholds are set
far from the values they must separate rather than tuned to them, as
the comparisons below indicate.
Segmentation cuts the record wherever pressure rises more than
$0.03$~\pu{} above its running minimum. A refill lifts pressure by far
more than that within a single sample interval, so this threshold
separates refills from noise without any changepoint machinery. Three
exclusions then remove what cannot carry a rate: segments shorter than
$4$~hr and segments shallower than $0.10$~\pu{}, against median values
of $10.2$~hr and $0.27$~\pu{}, and the first $3$~min of every segment,
which is the refill itself and therefore violates the assumption that
refilling is instantaneous. Cleaning finally removes two contaminants
that the equipment operators identified: pipe-washing days, on which
the vessel is deliberately cycled to clean lines, and the refill
interval.

Why should \eqref{eq:le} take that particular form? Because the two
processes act in different places. The culture lives in the liquid and
consumes gas that is already dissolved, so the headspace never loses
gas to the biology directly---it loses gas only across the gas--liquid
interface. That one fact splits the descent into two stages. Just after
a refill the liquid is undersaturated, transfer across the interface is
fast, and it slows as saturation is approached: this is the exponential
term $A\,e^{-kt}$. Once the liquid is near saturation, transfer no
longer runs ahead of consumption but merely replaces what the culture
has removed, so the headspace drains at whatever rate the biology sets:
this is the linear term $-\rb t$. The two terms are added because they
describe two stages of one descent, not two sinks acting at once.

Writing them additively is also what keeps \rb{} visible, which is
worth stating because the alternative fails silently. Had the
biological sink been attached to the headspace instead, as in
$\dot{P}=-k(P-P_{eq})-\rb$, the rate would enter only through the
combination $P_{eq}-\rb/k$ and the equilibrium pressure would absorb it
entirely. The data prefer the additive form as well: by the Akaike
information criterion (AIC) it wins on $74.9\%$ of the analysed cycles,
against $4.8\%$ for a pure line and $20.3\%$ for a pure exponential.

Its degeneracy is not a defect of our implementation but a long-known
property of exponential fitting, whose non-uniqueness and extreme
sensitivity to the data Varah characterized
directly \cite{varah1985fitting}; in the language of
...
held fixed. Line~4 builds the design matrix $D(k)$ of
Eq.~\eqref{eq:design} for the current $k$, Line~5 solves the resulting
$3\times3$ least-squares problem in closed form, and Line~6 records the
residual sum of squares $S(k)$ of Eq.~\eqref{eq:profile}. Line~7 closes
...
Algorithms~2 and~3 both call three small subroutines, listed together
below so that each is defined before it is used. What separates the two
validations is not these routines but the numbers they are given.

\begin{figure*}[t]
\noindent\textbf{Subroutines called by Algorithms 2 and 3}

\begin{verbatim}
FitPhysical(t, y):
  for each k in K do
    fit P_eq + A exp(-k t) by least squares
  return the (P_eq, A, k) of smallest residual
ResidualSD(t, y, P_eq, A, k):
  r <- y - (P_eq + A exp(-k t))
  return sqrt(r.r / (n - 3))    // 3 fitted
DrawAR1(n, sigma, phi):
  s <- sigma sqrt(1 - phi^2)
  z(1) <- Normal(0, sigma)
  for j <- 2 to n do
    z(j) <- phi z(j-1) + Normal(0, s)
  return z
\end{verbatim}
\end{figure*}

$\mathrm{FitPhysical}$ is Algorithm~1 with the biological term deleted:
the same grid over $k$, but a design matrix of two columns instead of
three, so the fit returns the best account of the descent that a purely
physical model can give. $\mathrm{ResidualSD}$ then measures what that
account failed to explain, dividing by $n-3$ because three parameters
were fitted; the result is a single number per cycle, in the units of
pressure. $\mathrm{DrawAR1}$ returns a first-order autoregressive
sequence whose innovations are scaled by $\sigma\sqrt{1-\varphi^{2}}$
rather than by $\sigma$, which is what keeps the standard deviation of
the sequence itself at $\sigma$ regardless of $\varphi$; without that
factor a larger autocorrelation would silently inflate the noise as
well.

\subsection{Parameter-Degeneracy Assessment}

Across the retained cycles the fitted \rbh{} and \kh{} correlate at
$\rho=0.790$, which read naively looks like proof that the estimate is
...
a constant truth ($\beta=0$) returns $\rho=0.290\pm0.048$, and a truth
independent of $k$ returns $\rho=0.122\pm0.079$, against an observed
$0.790$

\begin{verbatim}
Input: per-cycle LE fits k, A, P_eq, t, sigma, phi
Parameter: truth r_b(.), R = 25, quantum q = 0.01
Output: baseline distributions of rho and m
for r <- 1 to R do
  for i <- 1 to 207 do
    e <- DrawAR1(n, sigma, 1.3 phi)   // from LE fit
    z <- P_eq + A exp(-k t) - r_b(k) t + e
    y <- q round(z / q)         // same quantum
    (rb, kk) <- Algorithm 1 (t, y)
  end for
  rho(r) <- Spearman(rb, kk)    // over the 207
  m(r) <- median(rb)
end for
...
rather than a single number, and Line~2 walks the $207$ retained
cycles inside each repetition. Line~3 calls $\mathrm{DrawAR1}$, which
returns a first-order autoregressive sequence at that cycle's own
residual scale rather than white noise, because the measured
residuals are not white. Line~4 assembles the synthetic
...
Over all $207$ cycles this yields one draw of the aggregate median
...
\begin{verbatim}
Input: the retained cycles (t, y), i = 1..207
Parameter: draws D, grid K, quantum q = 0.01
Output: reference distribution of the median
for i <- 1 to 207 do
  (P_eq, A, k) <- FitPhysical(t, y)  // r_b fixed 0
  sigma <- ResidualSD(t, y, P_eq, A, k)
end for
for d <- 1 to D do
  for i <- 1 to 207 do
    e <- DrawAR1(n, sigma, 1.3 phi)  // physics-only
    z <- P_eq + A exp(-k t) + e
...
Line~1 opens a first pass over the retained cycles, in which the
reference itself is constructed. Line~2 calls $\mathrm{FitPhysical}$,
which refits the cycle with the rate held at zero and returns the
$P_{eq}$, $A$ and $k$ that give the best purely physical account of
that particular descent. Line~3 calls $\mathrm{ResidualSD}$, which
returns the root-mean-square residual of that same fit with its three
fitted parameters removed from the degrees of freedom; that scalar is
how much scatter physics alone leaves behind on this cycle, and it
sets the amplitude of the synthetic noise added later at Line~7.
Line~4 closes the pass. Line~5 then
draws $D$ realisations of the reference and Line~6 walks the cycles
within each. Line~7 calls the same $\mathrm{DrawAR1}$ routine that
Algorithm~2 uses, but not with the same numbers: there the scale and
the autocorrelation come from the residuals of the full fit of
\eqref{eq:le}, whereas here they come from the residuals of the
physics-only fit at Line~2. The distinction matters and it runs in our
favour. A straight biological trend that the physics-only model cannot
represent is left in its residuals, so those residuals are both larger
and smoother: across the retained cycles the median scale rises from
$0.0056$ to $0.0064$~\pu{} and the median autocorrelation from $0.30$
to $0.48$. The reference is therefore granted more noise, and noise of
exactly the slowly varying kind that a linear term can be mistaken
for, than the alternative it is being compared against. Line~8
generates a trace from the physics-only fit, with no linear term
present at all. Line~9 applies the same quantization and
...
of Sect.~\ref{sec:dynamics} to the whole archive returns $351$ segments, of which $270$ are
distinct: the same logger files are stored in more than one folder,
and the copies are removed before anything is fitted. The distinct
descents were logged over periods that do not all share the same operating
...
condition-based exclusions moves the screened median from $0.0132$ to
$0.0126$~\ratu{}, and applying none of the three moves it to
$0.0120$~\ratu{}; the two shifts, $-4.6\%$ and $-8.8\%$, are both
...
exclusions leave is $239$ cycles, of which the curvature screen retains
$207$. The median descent runs for $10.2$~hr, with quartiles at $7.1$
and $21.6$~hr, and ends in a narrow band---three quarters of them
between $0.70$ and $0.80$~\pu{}---which is the refill threshold making
itself visible in a record that never states it.

Those $207$ cycles come from four logging periods whose individual
medians run from $0.0113$ to $0.0203$~\ratu{}, and dropping either of
the two largest periods moves the aggregate by $-13.6\%$ or $+26.6\%$.
That spread is wider than the $\pm 11.0\%$ accuracy that
Sect.~\ref{sec:degen} will attach to the calibrated rate, and the two
statements do not contradict each other: the interval says how
precisely the median of \emph{this} record is pinned down, not how
steady the culture is from month to month. The
...
$0.0132$~\ratu{} before calibration.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figG_estimates.pdf}
\caption{What the estimator returns on the $207$ retained cycles.
...
per-cycle value is not: $42$ of the $207$ cycles return a negative
...
Table~\ref{tab:rate} collects the resulting figures alongside the
counts and the rate they lead to.
...
Distinct descents & $270$ & duplicate copies removed \\
Cycles analysed & $239$ & after condition exclusions \\
Cycles retained & $207$ & $c \ge 0.45$ \\
Calibrated \rb{} & $0.0125$~\ratu{} & $95\%$ $[0.0112,\ 0.0139]$ \\
\quad sampling component & $6.0\%$ & standard error, $207$ cycles \\
\quad systematic component & $\pm 2.2\%$ & range over the compatible set \\
Physics-only reference & $p=0.024$ & pure exponential truth \\
\midrule
Observed $\rho(\rbh,\kh)$ & $0.790$ & \\
Calibration, $\beta=0$ (constant) & $0.290 \pm 0.048$ & artefact level \\
Baseline, \rb{} independent of $k$ & $0.122 \pm 0.079$ & \\
Compatible set & $0.15 \le \beta \le 0.80$ & within calibration scatter \\
\midrule
Raw profiled median & $0.0132$~\ratu{} & before correction \\
Calibrated correction & $-4.8\%$ & estimator distortion only \\
\bottomrule
\end{tabular}
\end{table*}

A constant truth is excluded: $\beta=0$ produces only
$\rho=0.290 \pm 0.048$ against the observed $0.790$, a gap of roughly
ten standard deviations, and making \rb{} merely variable without
tying it to $k$ lowers the correlation further, to $0.122$. Truths
with $\beta$ between $0.15$ and $0.80$ reproduce the observed
...
with $\beta$ and reaches $30\%$ at the top of the compatible set. This
displacement must not be corrected, because it is not a distortion of
the estimate but a property of the quantity being estimated: what we
report is the rate \emph{on the cycles where it is identifiable}, and
the screen is what selects them. Correcting it would mean reporting a
rate for cycles on which no rate can be recovered. What must be
corrected is the estimator's own distortion, measured against
the truths actually kept, and that stays between $+2.1\%$ and
$+6.7\%$ across the whole compatible set---positive at every value of
$\beta$, so the estimator overstates the rate throughout. Inverting it corrects the raw
profiled median by $-4.8\%$, to $0.0125$~\ratu{}, at the centre of the
...
Monte Carlo, which shifts the centre slightly. The calibrated rate,
with the bracket giving its $95\%$ confidence interval, is
\begin{equation}
\rb = 0.0125\ [0.0112,\ 0.0139]~\ratu{} .
\label{eq:rate}
\end{equation}

That confidence interval combines two sources that behave differently. Resampling
the $207$ cycles gives a standard error of $6.0\%$ on the median, which
shrinks as logging continues; the spread over the compatible truths is
$\pm 2.2\%$ and does not, being set by how sharply the calibration
...
Monte Carlo. The two figures are quoted in different units---the first
is one standard error, the second a full range---so they do not add:
the Monte Carlo returns the $95\%$ half-width of $\pm 11.0\%$ in
\eqref{eq:rate}---the accuracy
...
white-Gaussian noise model is separated at $\mathrm{AUC}=0.954$; the
residuals of real cycles in fact carry a lag-1 autocorrelation with
median $0.237$. Modeling the noise as AR(1) reduces this to $0.766$,
and inflating $\hat{\varphi}$ by the factor $1.3$ calibrated on the
held-out split reaches $0.706$. All ten marginal trajectory features
then agree to within $0.34$ pooled standard deviations, the largest gap
being the monotone fraction ($0.815$ against $0.788$), with curvature,
residual scale, lag-1 autocorrelation, amplitude and duration all
within $0.18$; we return to the residual mismatch in
Sect.~\ref{sec:limits}.
...
trigger, so it should sit below the underlying trend. It does: across
the retained cycles the terminal residual averages $-0.0067$ of the
cycle amplitude, with a $95\%$ interval of $[-0.0115,\ -0.0019]$ that
excludes zero, against a mid-cycle mean indistinguishable from zero
with a spread of $0.0017$. Most of the effect has already been removed
by the screen, and not by design: on the unscreened descents the same
statistic is $-0.0675$, ten times larger, because a cycle too straight
to constrain $k$ is also one whose fit is least anchored at its ends.
Since a linear slope is most sensitive at its endpoints, what remains
is still a route to an inflated rate, though small enough to move the
slope by about a percent.

Trimming settles it. Removing the last $5$ to $90$ minutes of every
cycle moves the median \rb{} within $0.0125$--$0.0142$~\ratu{}, a
scatter of $\pm 6\%$ about the untrimmed value with no monotone trend,
and a design-matched simulation carrying no trigger selection at all
drifts over a comparable range as the same samples are removed. The
displacement is therefore a property of shortening the window rather
than of the trigger: one anomalous sample among roughly $610$ has too
little leverage to bend the slope.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figB_matched_baseline.pdf}
\caption{Calibration against the strength $\beta$ of the $k$
dependence, each point a full matched pipeline; the band in~(a) is
$\pm1$ standard deviation and the shading in both is the compatible
set. (a) A
constant truth ($\beta=0$) falls ten standard deviations below the
observation. (b) The estimator's own distortion, which we correct, and
the screen's selection of truths, which is definitional; inverting the
first gives the $-4.8\%$ correction.}
\label{fig:matched}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figH_null.pdf}
...
$0.00027$~\ratu{} and its largest draw reaches $0.0056$, against an
observed median of $0.0132$: no draw attains the observation, which
...
\subsection{An Independent Channel: Redox Potential}
...
The comparison has to be made per unit time. Taken over a whole cycle
the net change in redox potential is largely a clock: a long descent
accumulates more of it, and long descents also carry a lower fitted
rate, so ordering cycles by the cumulative change reproduces the
duration effect rather than any activity. Using the change per hour
instead, and comparing the aggregate rate across terciles, gives
$0.0120$, $0.0129$ and $0.0150$~\ratu{} from least to most active---a
$25\%$ span, in the direction the interpretation predicts.

That span is not resolved by the present record. Its bootstrap
interval, $[-0.0043,\ 0.0077]$, contains zero, so the ordering is
consistent with the redox channel tracking the same variation the
calibration found in \rb{}, but does not on its own establish it. We
report it as a direction that agrees rather than as a confirmation, and
the reason is the one the rest of the paper keeps meeting: ORP is a
mixed potential with no composition anchor, so what it supplies is a
ranking of limited precision, not a second measurement of the rate.
Sharpening it needs the chemical channel discussed in
Sect.~\ref{sec:limits}, not more cycles.
...
of Sect.~\ref{sec:null} excludes a purely physical explanation, but
what excludes physics does not by itself identify biology. That
attribution rests on the stoichiometry of the process, and on a redox
ordering that agrees in direction without resolving the size, rather
than on the pressure channel itself.
...
Eq.~\eqref{eq:rate}. Of the $\pm 11.0\%$ on the rate, the $\pm 2.2\%$
...
$\mathrm{AUC}=0.706$ even after all ten marginal features are matched,
...
alone does not reproduce the observed removal. Across $207$ screened
cycles the recovered rate is $0.0125$~\ratu{}, accurate to $11.0\%$, of
which $2.2\%$ is systematic and does not shrink with a longer record.
