...
const initMqtt = () => {
  try {
    ...
    mqttClient.on('error',   () => { isMqttConnected.value = false; systemStatus.value = '連線異常' })
    mqttClient.on('offline', () => { isMqttConnected.value = false; systemStatus.value = '等待重新連線...' })
  } catch {
    ...
    <header class="header">
      <div class="brand">
        ...
        <div class="status-group">
          <span class="dot" :class="{ connected: isMqttConnected, error: !isMqttConnected && systemStatus.includes('異常') }"></span>
